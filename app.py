...
def login():
    if request.method == 'POST':
        ...
        if user not in getUsers():
            error = 'Invalid username'
        if not validatePassword(user, request.form['password']):
...
def validatePassword(nickname, pass1):
    pass2 = query_db('SELECT Password FROM Spieler WHERE Nickname = ?', [nickname], one=True)
...
def setupAdmin():
    if ADMINLOGIN not in getUsers():
        update_db('INSERT INTO Spieler(SpielerID, Nickname) VALUES (?, ?)', [getSpielerID(), ADMINLOGIN])
    setPassword(ADMINLOGIN, ADMINPASSWORD)
    if MODLOGIN not in getUsers():
        update_db('INSERT INTO Spieler(SpielerID, Nickname) VALUES (?, ?)', [getSpielerID(), MODLOGIN])
    setPassword(MODLOGIN, MODPASSWORD)
